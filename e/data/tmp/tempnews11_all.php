...
<head>
<meta charset="utf-8">
<title><?=$grpagetitle?><?=ReturnPublicAddVar('SEOhz')?>-<?=$grpagetitle?><?=$ecms_gr[banben]?>-<?=$public_r['add_jianzhanzj_com_name']?></title>
<meta name="keywords" content="<?=$ecms_gr[keyboard]?>" />
<meta name="description" content="<?=$grpagetitle?><?=nl2br($ecms_gr[smalltext])?>" />
...
?>
    ...
    <div class="soft_related_box" id="arc_welike">
      ...
      <div class="show_top_list">
        <ul class="list"><?php
$bqno=0;
$ecms_bq_sql=sys_ReturnEcmsLoopBq('selfinfo',16,0,0,"softtype='".$navinfor['softtype']."'",'newstime DESC');
if($ecms_bq_sql){
while($bqr=$empire->fetch($ecms_bq_sql)){
$bqsr=sys_ReturnEcmsLoopStext($bqr);
$bqno++;
...
?>
    ...
    <div class="soft_related_box" id="arc_rmck">
	  ...
	  <?php
$bqno=0;
$ecms_bq_sql=sys_ReturnEcmsLoopBq('soft',10,18,0,'','newstime DESC');
if($ecms_bq_sql){
while($bqr=$empire->fetch($ecms_bq_sql)){
$bqsr=sys_ReturnEcmsLoopStext($bqr);
$bqno++;
