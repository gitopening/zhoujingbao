...
//---------- 伪静态 ----------
//游戏结合项页码
function gamepage($num,$line,$page_line,$start,$page,$search2){
	global $fun_r;
	$num=(int)$num;
	$line=(int)$line;
	$page_line=8;
	...
	$snum=2;//最小页数
	$totalpage=ceil($num/$line);//取得总页数
	$firststr='<span class=\'rows\'>共'.$num.'条</span>';
	//上一页
	if($page<>0)
	{
		$toppage='<a href="'.$url.$search.'0.html" class="firstPage on">'.$fun_r['startpage'].'</a>';
		$pagepr=$page-1;
		$prepage='<a href="'.$url.$search.$pagepr.'.html" class="prevPage on">'.$fun_r['pripage'].'</a>';
	}
	//下一页
	if($page!=$totalpage-1)
	{
		$pagenex=$page+1;
		$nextpage='<a href="'.$url.$search.$pagenex.'.html" class="nextPage on">'.$fun_r['nextpage'].'</a>';
		$lastpage='<a href="'.$url.$search.($totalpage-1).'.html" class="prevPage on">'.$fun_r['lastpage'].'</a>';
	}
	$starti=$page-$snum<0?0:$page-$snum;
	$no=0;
	for($i=$starti;$i<$totalpage&&$no<$page_line;$i++)
	{
		...
		{
			$is_1='<a href="'.$url.$search.$i.'.html" class="">';
			$is_2="</a>";
		}
		$pagenum=$i+1;
		$returnstr.=$is_1.$pagenum.$is_2;
...
function softpage($num,$line,$page_line,$start,$page,$search2){
	global $fun_r;
	$num=(int)$num;
	$line=(int)$line;
	$page_line=8;
...
function pcpage($num,$line,$page_line,$start,$page,$search2){
	global $fun_r;
	$num=(int)$num;
	$line=(int)$line;
	$page_line=8;
